...
function normalizeProductOptions(value) {
  ...
  const thirdLevelEnabled = nestedEnabled && value.thirdLevelEnabled === true;
  ...
  const items = (Array.isArray(value.items) ? value.items : []).map((item, index) => ({
    ...
    subOptions: (Array.isArray(item.subOptions) ? item.subOptions : []).map((subOption, subIndex) => ({
      ...
      subOptions: (Array.isArray(subOption.subOptions) ? subOption.subOptions : []).map((thirdOption, thirdIndex) => ({
        id: clean(thirdOption.id) || `third-option-${index + 1}-${subIndex + 1}-${thirdIndex + 1}`,
        nameAr: clean(thirdOption.nameAr || thirdOption.name),
        nameEn: clean(thirdOption.nameEn || thirdOption.nameAr || thirdOption.name),
        price: Math.max(0, Number(thirdOption.price) || 0)
      })).filter(thirdOption => thirdOption.nameAr && thirdOption.nameEn)
  ...
  const multipleStep = Math.min(stages, Math.max(1, Number(value.multipleStep) || 1));
  const quantityStep = Math.min(stages, Math.max(1, Number(value.quantityStep) || 1));
  return value.enabled !== false && items.length ? { enabled: true, required: nestedEnabled || value.required === true, multiple, multipleStep, maxSelections: multiple ? Math.max(1, Number(value.maxSelections) || items.length) : 1, titleAr: clean(value.titleAr), titleEn: clean(value.titleEn), priceBased, preparationEnabled: value.preparationEnabled === true, imagesEnabled: value.imagesEnabled === true, nestedEnabled, thirdLevelEnabled, minimumPerOptionEnabled: value.minimumPerOptionEnabled === true, optionQuantityEnabled: value.optionQuantityEnabled === true, quantityStep, items } : null;
}
...
async function openProductDialog(product = null, categoryId = "") {
  ...
  $("#productOptionsThirdLevelEnabled").checked = options?.thirdLevelEnabled === true;
...
function syncEditingProductOptions() {
  editingProductOptions.forEach((option, index) => {
    ...
    option.subOptions.forEach((subOption, subIndex) => {
      ...
      subOption.subOptions.forEach((thirdOption, thirdIndex) => {
        const thirdNameAr = $(`[data-third-option-ar="${index}-${subIndex}-${thirdIndex}"]`);
        const thirdNameEn = $(`[data-third-option-en="${index}-${subIndex}-${thirdIndex}"]`);
        const thirdPrice = $(`[data-third-option-price="${index}-${subIndex}-${thirdIndex}"]`);
        if (thirdNameAr) thirdOption.nameAr = thirdNameAr.value;
        if (thirdNameEn) thirdOption.nameEn = thirdNameEn.value;
        if (thirdPrice) thirdOption.price = Math.max(0, Number(thirdPrice.value) || 0);
      });
    });
  });
}
function renderProductOptions(syncFromForm = true) {
  ...
  const thirdLevelEnabled = nestedEnabled && $("#productOptionsThirdLevelEnabled").checked;
  const minimumPerOptionEnabled = $("#productOptionsMinimumEnabled").checked;
  const stages = thirdLevelEnabled ? 3 : nestedEnabled ? 2 : 1;
  const stageChoices = Array.from({ length: stages }, (_, index) => `<option value="${index + 1}">المرحلة ${index + 1}</option>`).join("");
  $("#productOptionsThirdLevelSetting").classList.toggle("hidden", !enabled || !nestedEnabled);
  $("#productOptionsStageSettings").classList.toggle("hidden", !enabled || stages === 1 || (!$("#productOptionsMultiple").checked && !$("#productOptionsQuantityEnabled").checked));
  $("#productOptionsMultipleStageWrap").classList.toggle("hidden", !enabled || stages === 1 || !$("#productOptionsMultiple").checked);
  $("#productOptionsQuantityStageWrap").classList.toggle("hidden", !enabled || stages === 1 || !$("#productOptionsQuantityEnabled").checked);
  [$("#productOptionsMultipleStep"), $("#productOptionsQuantityStep")].forEach(select => {
    const wanted = Math.min(stages, Math.max(1, Number(select.value) || 1));
    select.innerHTML = stageChoices;
...
function readProductOptions() {
  ...
  const items = editingProductOptions.map((option, index) => ({
    ...
    subOptions: (option.subOptions || []).map((subOption, subIndex) => ({
      ...
      subOptions: (subOption.subOptions || []).map((thirdOption, thirdIndex) => ({
        id: thirdOption.id || `third-option-${Date.now()}-${index}-${subIndex}-${thirdIndex}`,
        nameAr: clean($(`[data-third-option-ar="${index}-${subIndex}-${thirdIndex}"]`)?.value ?? thirdOption.nameAr),
        nameEn: clean($(`[data-third-option-en="${index}-${subIndex}-${thirdIndex}"]`)?.value ?? thirdOption.nameEn),
        price: Math.max(0, Number($(`[data-third-option-price="${index}-${subIndex}-${thirdIndex}"]`)?.value ?? thirdOption.price) || 0)
      }))
    }))
  ...
  if (thirdLevelEnabled && items.some(option => option.subOptions.some(subOption => !subOption.subOptions.length || subOption.subOptions.some(thirdOption => !thirdOption.nameAr || !thirdOption.nameEn)))) {
    toast("أضف اسماً عربياً وإنجليزياً لخيار واحد على الأقل في المرحلة الثالثة");
    return undefined;
  }
  const multiple = $("#productOptionsMultiple").checked;
  const stages = thirdLevelEnabled ? 3 : nestedEnabled ? 2 : 1;
  const multipleStep = Math.min(stages, Math.max(1, Number($("#productOptionsMultipleStep").value) || 1));
  const quantityStep = Math.min(stages, Math.max(1, Number($("#productOptionsQuantityStep").value) || 1));
  const maxSelections = Math.max(1, Math.min(99, Number(normalizeEnglishDigits($("#productOptionsMaxSelections").value)) || items.length));
  ...
  return { enabled: true, required: nestedEnabled || $("#productOptionsRequired").checked, multiple, multipleStep, maxSelections, titleAr: multiple ? clean($("#productOptionsTitleAr").value) : "", titleEn: multiple ? clean($("#productOptionsTitleEn").value) : "", priceBased: nestedEnabled || priceBased, preparationEnabled: $("#productOptionsPreparationEnabled").checked, imagesEnabled: $("#productOptionsImagesEnabled").checked, nestedEnabled, thirdLevelEnabled, minimumPerOptionEnabled, optionQuantityEnabled: $("#productOptionsQuantityEnabled").checked, quantityStep, items };
}
...
$("#productOptionsThirdLevelEnabled").addEventListener("change", event => {
  if (event.target.checked) {
    $("#productOptionsNestedEnabled").checked = true;
    $("#productOptionsRequired").checked = true;
    $("#productOptionsPriceBased").checked = true;
  }
  renderProductOptions();
...
$("#productOptionsMultipleStep").addEventListener("change", renderProductOptions);
$("#productOptionsQuantityStep").addEventListener("change", renderProductOptions);
